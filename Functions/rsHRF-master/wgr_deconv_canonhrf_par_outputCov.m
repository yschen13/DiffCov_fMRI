function [data_deconv event HRF adjust_global PARA Cov_collect] = wgr_deconv_canonhrf_par_outputCov(data,thr,event_lag_max,TR)
...
warning off
parfor i=1:nvar
    [data_deconv(:,i) HRF(:,i) event{i} adjust_global(i)  PARA(:,i) Cov_collect{i}] = wgr_adjust_onset(data(:,i),even_new{i},event_lag_max,TR,p_m,T,N);
	i
end
warning on
...
function [dat_deconv hrf even_new ad_global param Cov_E] = wgr_adjust_onset(dat,even_new,event_lag_max,TR,p_m,T,N)
%% global adjust.
kk=1;
